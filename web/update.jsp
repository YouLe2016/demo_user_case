...
<div class="container" style="width: 400px;">
    ...
    <form action="${pageContext.request.contextPath}/updateuser" method="post">
        ...
        <div class="form-group">
            <label>性别：</label>
            <input type="radio" name="sex" value="男" <c:if test="${user.gender == '男'}">checked</c:if>/>男
            <input type="radio" name="sex" value="女" <c:if test="${user.gender == '女'}">checked</c:if>/>女
        </div>
        ...
        <div class="form-group">
            ...
            <select name="address" class="form-control" id="address">
                ...
                <option value="河南" <c:if test="${user.address == '河南'}">selected</c:if>>河南</option>
            </select>
        </div>
